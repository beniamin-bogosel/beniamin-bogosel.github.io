...
export function loadBundle(bundle) {
  if(bundle?.format!=='spectral-shape-browser' || bundle.version!==1) throw Error('Expected a version 1 spectral browser model bundle. Export it using spectral_shape.export_browser.');
  ...
  if(!support||!validVector(support.scale,d,true)||!(support.radius>0)||!Number.isFinite(support.radius)||!(support.roundoffBound>=0&&support.roundoffBound<support.radius)||!Number.isInteger(support.count)||support.count<1||support.count>100000||support.rotations!==24) throw Error('Invalid training-support metadata.');
  if(!support.scale.slice(0,m).every((v,j)=>v===support.scale[j+m])) throw Error('Support scales must match within each sine/cosine pair.');
  if(!members[0].xscale.every((v,j)=>Math.abs((m>3?v:1)-support.scale[j])<1e-14)) throw Error('Support normalization does not match the model.');
  const q=decodeArray(support.q,support.count*d,'f64'), J=decodeArray(support.J,support.count*10,'f64');
  if(!J.every(v=>v>0)) throw Error('Training eigenvalues must be positive.');
  for(let i=0;i<support.count;i++) {
    let sum=0;
    for(let j=0;j<m;j++) {const a=Math.hypot(q[i*d+j],q[i*d+j+m]);sum+=a;if(a>bundle.caps[j]+1e-9)throw Error('Training sample exceeds a harmonic cap.');}
    if(sum>bundle.rho+1e-9) throw Error('Training sample exceeds the radial amplitude budget.');
  }
  return {bundle,members,q,J,dimension:d,modes:m};
}
