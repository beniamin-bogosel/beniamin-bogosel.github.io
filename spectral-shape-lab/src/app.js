...
let presets=[],saved=[];
try{saved=JSON.parse(localStorage.getItem('spectral-objectives-v1')||'[]');if(!Array.isArray(saved))saved=[];saved=saved.filter(p=>typeof p?.expression==='string'&&typeof p.name==='string').slice(0,30);}catch{saved=[];}
...
function setBusy(value) {busy=value;$('settings').disabled=value||!ready;$('model-file').disabled=value;$('run').disabled=value||!ready;$('pause').disabled=!value;$('stop').disabled=!value;}
...
function currentRecord() {return result||{format:'spectral-shape-snapshot',version:1,appVersion:'1.0.0',createdAt:new Date().toISOString(),status:$('run-state').textContent.toLowerCase(),model:metadata,settings:activeSettings,selection:'surrogate only; no PDE solve',best,current,trace};}
...
worker.onmessage=({data})=>{
  ...
  if(data.type==='loaded') {
    ...
    $('model-name').textContent=metadata.name;$('model-details').textContent=`${metadata.dimensions.join(' → ')} · ${metadata.members} members · ${metadata.trainingRows.toLocaleString()} training shapes · ${(metadata.bytes/1e6).toFixed(1)} MB`;
    ...
    $('constraint-details').textContent=`r(θ) / a₀ ≥ ${(1-metadata.rho).toFixed(2)} at every angle. Harmonic caps and training support stay active.`;
...
$('save-formula').addEventListener('click',()=>{
  ...
  try{localStorage.setItem('spectral-objectives-v1',JSON.stringify(saved));fillPresets(f.source);$('formula-feedback').textContent='Saved in this browser.';}catch{$('formula-feedback').textContent='Browser storage is unavailable. The current formula can still be used.';}
});
...
$('model-file').addEventListener('change',async()=>{
  const file=$('model-file').files[0];if(!file)return;
  if(file.size>64*1024*1024){showError('Model bundle exceeds 64 MB.');return;}
  ready=false;metadata=null;disk=null;resetResult();setBusy(true);$('pause').disabled=true;$('stop').disabled=true;showError('');$('model-state').textContent='Loading';$('model-name').textContent=file.name;
  worker.postMessage({type:'load',text:await file.text(),name:file.name});$('model-file').value='';
});
...
async function initialize() {
  ...
  worker.postMessage({type:'load',url:new URL('../models/vector10.spectral.json',import.meta.url).href});
}
initialize();
